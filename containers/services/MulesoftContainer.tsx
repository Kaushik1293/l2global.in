'use client'
import HeroCTA from '@/components/web/HeroCTA'
import ServiceAboutSection from '@/components/web/services/ServiceAboutSection'
import ServiceExpertiseSection from '@/components/web/services/ServiceExpertiseSection'
import ServiceHeroSection from '@/components/web/services/ServiceHeroSection'
import ServiceKPISection from '@/components/web/services/ServiceKPISection'
import ServiceProcessSteps from '@/components/web/services/ServiceProcessSteps'
import ServiceServedIndustries from '@/components/web/services/ServiceServedIndustries'
import ServiceWhyChooseUs from '@/components/web/services/ServiceWhyChooseUs'
import React from 'react'
import { Database, Cloud, GitMerge, BarChart3, Shield, Zap, } from "lucide-react";
import MulesoftImg from '../../public/assets/web/Service-detail/mulesoft-service-img.png'
import MulesoftAbout from '../../public/assets/web/Service-detail/Mulesoft-About.png'
import ServiceRegions from '@/components/web/services/ServiceRegions'
import ServiceFAQ from '@/components/web/services/ServiceFAQ'


const stats = [
    {
        icon: (
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" viewBox="0 0 16 16">
                <path d="M8 8c.828 0 1.5-.895 1.5-2S8.828 4 8 4s-1.5.895-1.5 2S7.172 8 8 8" />
                <path d="M11.953 8.81c-.195-3.388-.968-5.507-1.777-6.819C9.707 1.233 9.23.751 8.857.454a3.5 3.5 0 0 0-.463-.315A2 2 0 0 0 8.25.064.55.55 0 0 0 8 0a.55.55 0 0 0-.266.073 2 2 0 0 0-.142.08 4 4 0 0 0-.459.33c-.37.308-.844.803-1.31 1.57-.805 1.322-1.577 3.433-1.774 6.756l-1.497 1.826-.004.005A2.5 2.5 0 0 0 2 12.202V15.5a.5.5 0 0 0 .9.3l1.125-1.5c.166-.222.42-.4.752-.57.214-.108.414-.192.625-.281l.198-.084c.7.428 1.55.635 2.4.635s1.7-.207 2.4-.635q.1.044.196.083c.213.09.413.174.627.282.332.17.586.348.752.57l1.125 1.5a.5.5 0 0 0 .9-.3v-3.298a2.5 2.5 0 0 0-.548-1.562zM12 10.445v.055c0 .866-.284 1.585-.75 2.14.146.064.292.13.425.199.39.197.8.46 1.1.86L13 14v-1.798a1.5 1.5 0 0 0-.327-.935zM4.75 12.64C4.284 12.085 4 11.366 4 10.5v-.054l-.673.82a1.5 1.5 0 0 0-.327.936V14l.225-.3c.3-.4.71-.664 1.1-.861.133-.068.279-.135.425-.199M8.009 1.073q.096.06.226.163c.284.226.683.621 1.09 1.28C10.137 3.836 11 6.237 11 10.5c0 .858-.374 1.48-.943 1.893C9.517 12.786 8.781 13 8 13s-1.517-.214-2.057-.607C5.373 11.979 5 11.358 5 10.5c0-4.182.86-6.586 1.677-7.928.409-.67.81-1.082 1.096-1.32q.136-.113.236-.18Z" />
                <path d="M9.479 14.361c-.48.093-.98.139-1.479.139s-.999-.046-1.479-.139L7.6 15.8a.5.5 0 0 0 .8 0z" />
            </svg>
        ),
        number: "250+",
        title: "APIs Delivered",
        subtitle: "Enterprise-wide integrations",
        gradient: "blue",
    },
    {
        icon: (
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" viewBox="0 0 16 16">
                <path d="M7 14s-1 0-1-1 1-4 5-4 5 3 5 4-1 1-1 1zm4-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6m-5.784 6A2.24 2.24 0 0 1 5 13c0-1.355.68-2.75 1.936-3.72A6.3 6.3 0 0 0 5 9c-4 0-5 3-5 4s1 1 1 1zM4.5 8a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5" />
            </svg>
        ),
        number: "99.9%",
        title: "System Uptime",
        subtitle: "Reliable real-time connectivity",
        gradient: "purple",
    },
    {
        icon: (
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" viewBox="0 0 16 16">
                <path d="M9.669.864 8 0 6.331.864l-1.858.282-.842 1.68-1.337 1.32L2.6 6l-.306 1.854 1.337 1.32.842 1.68 1.858.282L8 12l1.669-.864 1.858-.282.842-1.68 1.337-1.32L13.4 6l.306-1.854-1.337-1.32-.842-1.68zm1.196 1.193.684 1.365 1.086 1.072L12.387 6l.248 1.506-1.086 1.072-.684 1.365-1.51.229L8 10.874l-1.355-.702-1.51-.229-.684-1.365-1.086-1.072L3.614 6l-.25-1.506 1.087-1.072.684-1.365 1.51-.229L8 1.126l1.356.702z" />
                <path d="M4 11.794V16l4-1 4 1v-4.206l-2.018.306L8 13.126 6.018 12.1z" />
            </svg>
        ),
        number: "40%",
        title: "Faster Integrations",
        subtitle: "Accelerated deployment cycles",
        gradient: "orange",
    },
    {
        icon: (
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" viewBox="0 0 16 16">
                <path d="M8 1a5 5 0 0 0-5 5v1h1a1 1 0 0 1 1 1v3a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V6a6 6 0 1 1 12 0v6a2.5 2.5 0 0 1-2.5 2.5H9.366a1 1 0 0 1-.866.5h-1a1 1 0 1 1 0-2h1a1 1 0 0 1 .866.5H11.5A1.5 1.5 0 0 0 13 12h-1a1 1 0 0 1-1-1V8a1 1 0 0 1 1-1h1V6a5 5 0 0 0-5-5" />
            </svg>
        ),
        number: "24/7",
        title: "Integration Monitoring",
        subtitle: "Enterprise-grade support",
        gradient: "green",
    },
];

const features = [
    "API-led connectivity with reusable integration frameworks",
    "Seamless integration across cloud, on-premise, and hybrid systems",
    "Real-time data synchronization between enterprise applications",
    "Secure API management, governance, and monitoring",
    "Scalable architectures built for long-term growth",
];


const services = [
    {
        icon: Database,
        title: "API-Led Connectivity",
        description:
            "Design and implement reusable, scalable APIs using MuleSoft’s API-led architecture to accelerate integration across systems and teams.",
        iconBg: "bg-blue-100",
        gradientBar: "from-blue-500 to-blue-300",
    },
    {
        icon: Cloud,
        title: "Cloud & Hybrid Integration",
        description:
            "Seamlessly connect cloud, on-premise, and hybrid applications using Anypoint Platform for secure and scalable data exchange.",
        iconBg: "bg-green-100",
        gradientBar: "from-green-500 to-green-300",
    },
    {
        icon: GitMerge,
        title: "Enterprise System Integration",
        description:
            "Integrate Salesforce, SAP, Oracle, AWS, and other enterprise systems using APIs, connectors, and event-driven architecture.",
        iconBg: "bg-orange-100",
        gradientBar: "from-orange-500 to-orange-300",
    },
    {
        icon: BarChart3,
        title: "API Management & Governance",
        description:
            "Manage the full API lifecycle with secure publishing, monitoring, versioning, and policy enforcement across your organization.",
        iconBg: "bg-purple-100",
        gradientBar: "from-purple-500 to-purple-300",
    },
    {
        icon: Shield,
        title: "Security & Compliance",
        description:
            "Implement role-based access, OAuth policies, encryption, and governance frameworks to ensure secure data transmission and compliance.",
        iconBg: "bg-yellow-100",
        gradientBar: "from-yellow-500 to-yellow-300",
    },
    {
        icon: Zap,
        title: "Performance Optimization",
        description:
            "Optimize SAP performance through Basis administration, code tuning, and infrastructure enhancement strategies.",
        iconBg: "bg-cyan-100",
        gradientBar: "from-cyan-500 to-cyan-300",
    },
];

const mulesoftFaqs = [
    {
        q: 'Where can I find a MuleSoft consultant near me?',
        a: 'L2 Global Technologies provides certified MuleSoft consultants across UK (London, Manchester), USA (New York, Texas) and Gulf (Dubai, Riyadh). Remote delivery. Free consultation at l2global.in/contact-us.'
    },
    {
        q: 'How much does MuleSoft integration cost?',
        a: 'Single integration: £10,000–£40,000. Enterprise programme: £40,000–£200,000+. L2 Global offers transparent pricing. Free consultation available.'
    },
    {
        q: 'Do you offer MuleSoft services in Dubai?',
        a: 'Yes. L2 Global serves UAE and Gulf enterprises with MuleSoft implementation, API design and 24/7 integration monitoring.'
    },
    {
        q: 'How long does a MuleSoft project take?',
        a: 'Single integration: 4–10 weeks. Enterprise programme: 3–9 months. Our proven agile methodology delivers 40% faster than industry average.'
    },
];

const MulesoftContainer = () => {
    return (
        <div>
            <ServiceHeroSection
                image={MulesoftImg}
                sectionTitle={'Enterprise Integration'}
                titleBefore={'Scalable & Secure'}
                titleAfter={'Solutions'}
                linearText={'MuleSoft'}
                description={'Certified MuleSoft Anypoint Platform consultants serving businesses in London, Manchester, New York, Texas, Dubai and across the UAE. API-led connectivity, enterprise integration, and 24/7 monitoring. 250+ APIs delivered. Free consultation.'}
                tag1={'API-Led Architecture'} tag2={'Grade Security'} tag3={'24/7 Support'} />

            <ServiceKPISection stats={stats} />

            <ServiceAboutSection image={MulesoftAbout} titleBefore={'Why'} titleAfter={'Integration Matters'} linearText={'Mulesoft'}
                description={'MuleSoft enables seamless connectivity across applications, data sources, and devices - creating a unified, API-led ecosystem. By breaking down system silos, organizations gain real-time visibility, automation, and scalable integration capabilities across cloud and on-premise environments.'} features={features} />

            <ServiceExpertiseSection services={services} />

            <ServiceProcessSteps />

            <ServiceServedIndustries />

            <ServiceWhyChooseUs />

            <ServiceRegions serviceName='MuleSoft Integration Services' />

            <ServiceFAQ faqs={mulesoftFaqs} serviceName='MuleSoft Integration Services' />

            <div className=' pt-2 md:pt-8 lg:pt-12'>
                <HeroCTA
                    tag="Let's Grow Together"
                    heading="Ready to Connect Every System?"
                    description="Partner with L2 Global for MuleSoft Anypoint implementation and API-led connectivity across Salesforce, SAP and beyond."
                    primaryBtnText="Discuss Your Integration"
                    primaryBtnLink="/contact-us"
                    secondaryBtnText="View Services"
                    secondaryBtnLink="/services"
                />

            </div>
        </div>
    )
}

export default MulesoftContainer